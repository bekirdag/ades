...
SMOKE_TAG_BATCH_SUMMARY_ITEM_COUNT_FIELDS = (
    "discovered_count",
    "included_count",
    "processed_count",
)
SMOKE_TAG_BATCH_ZERO_STATE_SUMMARY_COUNT_FIELDS = (
    "excluded_count",
    "skipped_count",
    "rejected_count",
    "limit_skipped_count",
...
SMOKE_TAG_BATCH_ZERO_STATE_DUPLICATE_GENERATED_COUNT_FIELDS = (
    "duplicate_count",
    "generated_output_skipped_count",
)
...
def _parse_batch_source_paths(result: ReleaseCommandResult) -> list[str]:
    """Extract item source paths reported by one JSON `ades tag-files` payload."""
...
def _parse_batch_source_fingerprints(
    result: ReleaseCommandResult,
) -> list[dict[str, object]]:
    """Extract item source fingerprints reported by one JSON `ades tag-files` payload."""
    ...
    for item in items:
        ...
        modified_time_ns = source_fingerprint.get("modified_time_ns")
        sha256 = source_fingerprint.get("sha256")
        if (
            isinstance(size_bytes, int)
            and not isinstance(size_bytes, bool)
            and isinstance(modified_time_ns, int)
            and not isinstance(modified_time_ns, bool)
...
def _expected_smoke_batch_source_paths(working_dir: Path) -> list[str]:
    return [
...
def _expected_smoke_batch_summary_item_counts() -> dict[str, int]:
    total_items = len(SMOKE_TAG_BATCH_FILES)
    return {
        field_name: total_items for field_name in SMOKE_TAG_BATCH_SUMMARY_ITEM_COUNT_FIELDS
    }


def _parse_batch_summary_item_counts(
    result: ReleaseCommandResult,
) -> dict[str, int] | None:
    summary_counts: dict[str, int] = {}
    for field_name in SMOKE_TAG_BATCH_SUMMARY_ITEM_COUNT_FIELDS:
        value = _parse_batch_summary_int_value(result, field_name)
        if value is None:
            return None
        summary_counts[field_name] = value
    return summary_counts


def _expected_smoke_batch_zero_state_summary_counts() -> dict[str, int]:
    return {
        field_name: 0 for field_name in SMOKE_TAG_BATCH_ZERO_STATE_SUMMARY_COUNT_FIELDS
    }


def _parse_batch_zero_state_summary_counts(
    result: ReleaseCommandResult,
) -> dict[str, int] | None:
...
def _expected_smoke_batch_zero_state_duplicate_generated_counts() -> dict[str, int]:
    return {
        field_name: 0
        for field_name in SMOKE_TAG_BATCH_ZERO_STATE_DUPLICATE_GENERATED_COUNT_FIELDS
    }


def _parse_batch_zero_state_duplicate_generated_counts(
    result: ReleaseCommandResult,
) -> dict[str, int] | None:
    zero_state_counts: dict[str, int] = {}
    for field_name in SMOKE_TAG_BATCH_ZERO_STATE_DUPLICATE_GENERATED_COUNT_FIELDS:
...
def _source_fingerprint_payload(
    ...
    *,
    ...
    if fallback_content is None:
        raise FileNotFoundError(f"Smoke source file does not exist: {resolved_path}")
    payload = fallback_content.encode("utf-8")
    return {
        "size_bytes": len(payload),
        "modified_time_ns": SMOKE_TAG_BATCH_FAKE_MODIFIED_TIME_NS,
        "sha256": hashlib.sha256(payload).hexdigest(),
    }


def _expected_smoke_batch_source_fingerprints(working_dir: Path) -> list[dict[str, object]]:
    return [
...
def _source_fingerprints_match_expected_content(
    source_fingerprints: list[dict[str, object]],
) -> bool:
    if len(source_fingerprints) != len(SMOKE_TAG_BATCH_FILES):
        return False
    for source_fingerprint, (_, content) in zip(source_fingerprints, SMOKE_TAG_BATCH_FILES):
        size_bytes = source_fingerprint.get("size_bytes")
        modified_time_ns = source_fingerprint.get("modified_time_ns")
        sha256 = source_fingerprint.get("sha256")
        expected_payload = content.encode("utf-8")
        if (
            size_bytes != len(expected_payload)
            or not isinstance(modified_time_ns, int)
            or isinstance(modified_time_ns, bool)
            or sha256 != hashlib.sha256(expected_payload).hexdigest()
        ):
            return False
    return True


def _source_fingerprints_have_matching_modified_times(
    ...
    if len(left) != len(right):
        return False
    return all(
        left_item.get("modified_time_ns") == right_item.get("modified_time_ns")
...
def _run_python_install_smoke(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="ades-wheel-smoke-") as temp_dir:
        ...
        serve_tag_files_summary_item_counts = _parse_batch_summary_item_counts(
            serve_tag_files
        )
        ...
        serve_tag_files_zero_state_duplicate_generated_counts = (
            _parse_batch_zero_state_duplicate_generated_counts(serve_tag_files)
        )
        ...
        expected_serve_tag_files_summary_item_counts = (
            _expected_smoke_batch_summary_item_counts()
        )
        ...
        expected_serve_tag_files_zero_state_duplicate_generated_counts = (
            _expected_smoke_batch_zero_state_duplicate_generated_counts()
        )
        ...
        has_expected_serve_tag_files_summary_item_counts = (
            serve_tag_files_summary_item_counts == expected_serve_tag_files_summary_item_counts
        )
        has_expected_serve_tag_files_zero_state_summary_counts = (
            serve_tag_files_zero_state_summary_counts
            == expected_serve_tag_files_zero_state_summary_counts
        )
        ...
        has_expected_serve_tag_files_zero_state_duplicate_generated_counts = (
            serve_tag_files_zero_state_duplicate_generated_counts
            == expected_serve_tag_files_zero_state_duplicate_generated_counts
        )
        ...
        has_expected_serve_tag_files_replay_summary_item_counts = (
            _parse_batch_summary_item_counts(serve_tag_files_replay)
            == expected_serve_tag_files_summary_item_counts
        )
        ...
        has_expected_serve_tag_files_replay_zero_state_duplicate_generated_counts = (
            _parse_batch_zero_state_duplicate_generated_counts(serve_tag_files_replay)
            == expected_serve_tag_files_zero_state_duplicate_generated_counts
        )
        ...
        passed = (
            ...
            and has_expected_serve_tag_files_zero_state_duplicate_generated_counts
            ...
            and has_expected_serve_tag_files_replay_zero_state_duplicate_generated_counts
...
def _smoke_install_warnings(
    ...
    *,
    ...
    serve_tag_files_source_paths = _parse_batch_source_paths(smoke_result.serve_tag_files)
    expected_serve_tag_files_source_paths = _expected_smoke_batch_source_paths(
        Path(smoke_result.working_dir)
    )
    ...
    if serve_tag_files_summary_item_counts != expected_serve_tag_files_summary_item_counts:
        return [f"{prefix}_serve_tag_files_invalid_summary_item_counts"]
    serve_tag_files_zero_state_summary_counts = _parse_batch_zero_state_summary_counts(
        smoke_result.serve_tag_files
    )
    expected_serve_tag_files_zero_state_summary_counts = (
        _expected_smoke_batch_zero_state_summary_counts()
    ...
    serve_tag_files_zero_state_duplicate_generated_counts = (
        _parse_batch_zero_state_duplicate_generated_counts(smoke_result.serve_tag_files)
    )
    expected_serve_tag_files_zero_state_duplicate_generated_counts = (
        _expected_smoke_batch_zero_state_duplicate_generated_counts()
    )
    if (
        ...
        return [
            f"{prefix}_serve_tag_files_invalid_zero_state_duplicate_generated_counts"
        ]
    serve_tag_files_summary_input_bytes = _parse_batch_summary_input_bytes(
        smoke_result.serve_tag_files
    )
    ...
    if not _source_fingerprints_match_expected_content(serve_tag_files_source_fingerprints):
        return [f"{prefix}_serve_tag_files_invalid_source_fingerprints"]
    ...
    serve_tag_files_replay_summary_item_counts = _parse_batch_summary_item_counts(
        smoke_result.serve_tag_files_replay
    )
    if (
        serve_tag_files_replay_summary_item_counts
        != expected_serve_tag_files_summary_item_counts
    ):
        return [f"{prefix}_serve_tag_files_replay_invalid_summary_item_counts"]
    serve_tag_files_replay_zero_state_summary_counts = (
        _parse_batch_zero_state_summary_counts(smoke_result.serve_tag_files_replay)
    )
    if (
        serve_tag_files_replay_zero_state_summary_counts
        != expected_serve_tag_files_zero_state_summary_counts
    ...
    serve_tag_files_replay_zero_state_duplicate_generated_counts = (
        _parse_batch_zero_state_duplicate_generated_counts(
            smoke_result.serve_tag_files_replay
        )
    )
    if (
        ...
        return [
            f"{prefix}_serve_tag_files_replay_invalid_zero_state_duplicate_generated_counts"
        ]
    ...
    if (
        not _source_fingerprints_match_expected_content(
            serve_tag_files_replay_source_fingerprints
        )
        or not _source_fingerprints_have_matching_modified_times(
            serve_tag_files_source_fingerprints,
            serve_tag_files_replay_source_fingerprints,
        )
    ):
        return [f"{prefix}_serve_tag_files_replay_invalid_source_fingerprints"]

...
@dataclass(frozen=True)
class ImpactArticleGoldenCase:
    """One article-text impact expansion evaluation case."""
...
@dataclass(frozen=True)
class ImpactArticleGoldenSet:
    """An article-text impact expansion golden set."""

    name: str
    description: str | None = None
    default_pack: str | None = None
    default_enabled_packs: tuple[str, ...] = ()
    cases: tuple[ImpactArticleGoldenCase, ...] = ()
...
@dataclass(frozen=True)
class ImpactArticleGoldenCaseResult:
    """Evaluation result for one article-text impact golden case."""

    name: str
    pack: str | None
    expected_entity_refs: list[str] = field(default_factory=list)
    extracted_entity_refs: list[str] = field(default_factory=list)
...
@dataclass(frozen=True)
class ImpactArticleGoldenSetReport:
    """Aggregate article-text impact expansion golden-set report."""
    ...
    per_relation_family_false_positives: dict[str, int] = field(default_factory=dict)
    warnings: list[str] = field(default_factory=list)
    cases: list[ImpactArticleGoldenCaseResult] = field(default_factory=list)

    @property
    def passed(self) -> bool:
        """Return whether every article golden case passed its expected outcome."""
...
def load_impact_article_golden_set(path: str | Path) -> ImpactArticleGoldenSet:
    """Load one article-text impact expansion golden set from JSON."""

    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("Impact article golden set must be a JSON object.")
    cases_payload = payload.get("cases")
    if not isinstance(cases_payload, list) or not cases_payload:
        raise ValueError("Impact article golden set must include at least one case.")
    cases: list[ImpactArticleGoldenCase] = []
    for index, item in enumerate(cases_payload, start=1):
        if not isinstance(item, dict):
            raise ValueError(f"Impact article golden case {index} must be an object.")
        name = str(item.get("name") or "").strip()
        text = str(item.get("text") or "").strip()
        if not name or not text:
            raise ValueError(f"Impact article golden case {index} is missing name or text.")
        pack = str(item.get("pack") or "").strip() or None
        cases.append(
            ImpactArticleGoldenCase(
                name=name,
                text=text,
                expected_entity_refs=_string_tuple(item.get("expected_entity_refs")),
                expected_candidate_refs=_string_tuple(item.get("expected_candidate_refs")),
                expected_passive_refs=_string_tuple(item.get("expected_passive_refs")),
                expected_relation_families=_string_tuple(
                    item.get("expected_relation_families")
    ...
    return ImpactArticleGoldenSet(
        name=str(payload.get("name") or Path(path).stem),
        description=str(payload.get("description") or "").strip() or None,
        default_pack=str(payload.get("default_pack") or "").strip() or None,
        default_enabled_packs=_string_tuple(payload.get("default_enabled_packs")),
        cases=tuple(cases),
    )
...
def _linked_entity_refs(tag_response: Any) -> tuple[str, ...]:
    refs: list[str] = []
    seen: set[str] = set()
    for entity in getattr(tag_response, "entities", []):
        link = getattr(entity, "link", None)
        entity_ref = str(getattr(link, "entity_id", "") or "").strip()
        if entity_ref and entity_ref not in seen:
...
def evaluate_impact_article_golden_set(
    *,
    ...
    golden_set = load_impact_article_golden_set(golden_set_path)
    resolved_tagger = tagger
    if resolved_tagger is None:
        raise ValueError("A tagger callable is required for article impact evaluation.")

    case_results: list[ImpactArticleGoldenCaseResult] = []
    latencies: list[int] = []
    expected_relation_counts: Counter[str] = Counter()
    matched_relation_counts: Counter[str] = Counter()
    false_positive_relations: Counter[str] = Counter()
    ...
    for case in golden_set.cases:
        packs = list(case.enabled_packs or default_packs)
        pack = case.pack or default_pack or golden_set.default_pack
        tag_kwargs: dict[str, Any] = {}
        if pack is not None:
            tag_kwargs["pack"] = pack
        if storage_root is not None:
            tag_kwargs["storage_root"] = storage_root

        start = time.perf_counter()
        tag_response = resolved_tagger(case.text, **tag_kwargs)
        extracted_refs = _linked_entity_refs(tag_response)
        result = expand_impact_paths(
        ...
        expected_entities = set(case.expected_entity_refs)
        expected_candidates = set(case.expected_candidate_refs)
        expected_passives = set(case.expected_passive_refs)
        expected_relations = set(case.expected_relation_families)
        extracted_entity_set = set(extracted_refs)
        actual_candidate_set = set(actual_candidates)
        actual_passive_set = set(actual_passives)

        missing_entities = sorted(expected_entities - extracted_entity_set)
        missing_candidates = sorted(expected_candidates - actual_candidate_set)
        missing_passives = sorted(expected_passives - actual_passive_set)
        unexpected_candidates = sorted(actual_candidate_set - expected_candidates)
        unexpected_passives = sorted(actual_passive_set - expected_passives)
        ...
        matched_entity_count += len(expected_entities & extracted_entity_set)
        for relation in expected_relations:
            expected_relation_counts[relation] += 1
            if relation in actual_relations:
                matched_relation_counts[relation] += 1
        ...
        tag_warnings = list(getattr(tag_response, "warnings", []) or [])
        if tag_warnings:
            warnings.extend(f"{case.name}:tag:{warning}" for warning in tag_warnings)
        if result.warnings:
            warnings.extend(f"{case.name}:impact:{warning}" for warning in result.warnings)

        passed = (
            not missing_entities
            and not missing_candidates
            and not missing_passives
        ...
        case_results.append(
            ImpactArticleGoldenCaseResult(
                name=case.name,
                pack=pack,
                expected_entity_refs=list(case.expected_entity_refs),
                extracted_entity_refs=list(extracted_refs),
                ...
                missing_relation_families=missing_relations,
                unexpected_relation_families=unexpected_relations,
                warning_count=len(tag_warnings) + len(result.warnings),
                timing_ms=elapsed_ms,
                negative=case.negative,
                passed=passed,
            )
    ...
    extraction_recall = (
        round(matched_entity_count / expected_entity_count, 4)
        if expected_entity_count
        else 1.0
    )
    case_count = len(golden_set.cases)
    return ImpactArticleGoldenSetReport(
        name=golden_set.name,
        case_count=case_count,

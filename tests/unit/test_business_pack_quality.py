...
from ades.packs.business_quality import validate_business_pack_quality


def _build_business_bundle(tmp_path: Path) -> str:
    bundles = build_bdya_domain_source_bundles(output_dir=tmp_path / "bundles")
    return next(
        bundle.bundle_dir
        for bundle in bundles
        if bundle.pack_id == "business-vector-en"
    )


def test_validate_business_pack_quality_reports_passing_fixture_metrics(
    tmp_path: Path,
) -> None:
    bundle_dir = _build_business_bundle(tmp_path)

    report = validate_business_pack_quality(
        bundle_dir,
        output_dir=tmp_path / "quality-output",
    )

    assert report.pack_id == "business-vector-en"
    ...
    assert report.actual_entity_count == 13
    assert report.matched_expected_entity_count == 13
    ...
    assert report.alias_count == 82
    assert report.unique_canonical_count == 19
    ...
    negative_case = next(
        ...
        if case.name == "blocked-shortforms-and-generic-prose"
    )
    assert negative_case.actual_entities == []


def test_validate_business_pack_quality_supports_smoke_profile(
    tmp_path: Path,
) -> None:
    bundle_dir = _build_business_bundle(tmp_path)
    ...
    assert report.expected_entity_count == 3
    assert report.passed is True

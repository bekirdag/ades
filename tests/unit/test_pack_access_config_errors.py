...
def test_api_get_pack_rejects_missing_explicit_config(
...
def test_api_get_pack_rejects_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_api_pull_rejects_missing_explicit_config(tmp_path: Path, monkeypatch) -> None:
    missing_config = tmp_path / "missing.toml"
    monkeypatch.setenv("ADES_CONFIG_FILE", str(missing_config))
...
def test_api_pull_rejects_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
    config_path = tmp_path / "ades.toml"
    config_path.write_text("", encoding="utf-8")

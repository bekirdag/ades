...
def test_validate_general_pack_quality_reports_passing_fixture_metrics(tmp_path: Path) -> None:
    ...
    assert report.expected_entity_count == 8
    assert report.actual_entity_count == 8
    assert report.matched_expected_entity_count == 8
    ...
    assert report.alias_count == 8
    assert report.unique_canonical_count == 4

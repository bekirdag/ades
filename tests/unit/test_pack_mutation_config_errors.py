...
from ades.api import activate_pack, deactivate_pack, remove_pack
from ades.storage import UnsupportedRuntimeConfigurationError


def test_api_activate_pack_rejects_missing_explicit_config(
...
def test_api_deactivate_pack_rejects_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_api_remove_pack_rejects_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
    config_path = tmp_path / "ades.toml"
    config_path.write_text("", encoding="utf-8")

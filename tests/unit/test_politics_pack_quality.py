...
def _build_politics_bundle(tmp_path: Path) -> str:
    ...
    return next(bundle.bundle_dir for bundle in bundles if bundle.pack_id == "politics-vector-en")
...
def test_validate_politics_pack_quality_reports_passing_fixture_metrics(
    ...
    assert report.expected_entity_count == 25
    assert report.actual_entity_count == 25
    assert report.matched_expected_entity_count == 25
    ...
    qid_case = next(case for case in report.cases if case.name == "wikidata-organization-anchors")
    assert {entity.label for entity in qid_case.actual_entities} == {"organization"}

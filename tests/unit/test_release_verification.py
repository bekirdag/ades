...
def _served_batch_manifest_payload(
    ...
    *,
    ...
    zero_state_duplicate_generated_counts: dict[str, int] | None = None,
    ...
    effective_zero_state_duplicate_generated_counts = (
        build_expected_batch_zero_state_duplicate_generated_counts()
        if zero_state_duplicate_generated_counts is None
        else zero_state_duplicate_generated_counts
    )
    ...
    assert len(effective_source_fingerprints) >= 2
    items: list[dict[str, object]] = [
        {
            ...
            "source_fingerprint": effective_source_fingerprints[0],
        ...
        },
        {
            ...
            "source_fingerprint": effective_source_fingerprints[1],
...
def test_verify_release_artifacts_reports_live_service_batch_summary_item_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_summary_counts = build_expected_batch_summary_item_counts()
        invalid_summary_counts["processed_count"] += 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
            batch_payload=_served_batch_manifest_payload(
                working_dir,
                version=expected_version,
                summary_counts=invalid_summary_counts,
            ),
        )
    ...
    assert response.npm_install_smoke.serve_tag_files is not None
    assert response.npm_install_smoke.serve_tag_files.passed is True
    assert response.warnings == ["npm_tarball_serve_tag_files_invalid_summary_item_counts"]


def test_verify_release_artifacts_reports_live_service_batch_zero_state_summary_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_zero_state_counts = build_expected_batch_zero_state_summary_counts()
        invalid_zero_state_counts["excluded_count"] = 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
            batch_payload=_served_batch_manifest_payload(
                working_dir,
                version=expected_version,
                zero_state_summary_counts=invalid_zero_state_counts,
            ),
        )
    ...
    assert response.npm_install_smoke.serve_tag_files.passed is True
    assert response.warnings == [
        "npm_tarball_serve_tag_files_invalid_zero_state_summary_counts"
    ]
...
def test_verify_release_artifacts_reports_live_service_batch_zero_state_duplicate_generated_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_zero_state_counts = (
            build_expected_batch_zero_state_duplicate_generated_counts()
        )
        invalid_zero_state_counts["duplicate_count"] = 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
            batch_payload=_served_batch_manifest_payload(
                working_dir,
                version=expected_version,
                zero_state_duplicate_generated_counts=invalid_zero_state_counts,
            ),
        )
    ...
    assert response.npm_install_smoke.serve_tag_files.passed is True
    assert response.warnings == [
        "npm_tarball_serve_tag_files_invalid_zero_state_duplicate_generated_counts"
    ]
...
def test_verify_release_artifacts_reports_live_service_batch_source_fingerprint_mismatches(
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_source_fingerprints = build_expected_batch_source_fingerprints()
        ...
        return _service_smoke_tuple(
            ...
            batch_payload=_served_batch_manifest_payload(
                working_dir,
                version=expected_version,
                source_fingerprints=invalid_source_fingerprints,
            ),
            replay_passed=False,
            replay_stderr="Skipped because live batch tagging wrote unexpected source fingerprints.",
        )
    ...
    assert response.npm_install_smoke.serve_tag_files is not None
    assert response.npm_install_smoke.serve_tag_files.passed is True
    assert response.warnings == ["npm_tarball_serve_tag_files_invalid_source_fingerprints"]
...
def test_verify_release_artifacts_reports_live_service_batch_replay_summary_item_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_summary_counts = build_expected_batch_summary_item_counts()
        invalid_summary_counts["discovered_count"] += 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
    ...
    assert response.npm_install_smoke.serve_tag_files_replay.passed is True
    assert response.warnings == [
        "npm_tarball_serve_tag_files_replay_invalid_summary_item_counts"
    ]


def test_verify_release_artifacts_reports_live_service_batch_replay_zero_state_summary_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_zero_state_counts = build_expected_batch_zero_state_summary_counts()
        invalid_zero_state_counts["limit_skipped_count"] = 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
...
def test_verify_release_artifacts_reports_live_service_batch_replay_zero_state_duplicate_generated_count_mismatches(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_root, npm_package_dir = create_release_project(tmp_path / "repo")
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_zero_state_counts["generated_output_skipped_count"] = 1
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
            expected_version=expected_version,
    ...
    assert response.npm_install_smoke.serve_tag_files_replay.passed is True
    assert response.warnings == [
        "npm_tarball_serve_tag_files_replay_invalid_zero_state_duplicate_generated_counts"
    ]
...
def test_verify_release_artifacts_reports_live_service_batch_replay_source_fingerprint_mismatches(
    ...
    def fake_service_smoke(*, executable, working_dir, storage_root, expected_version, extra_env=None):
        ...
        invalid_source_fingerprints[0] = {
            **invalid_source_fingerprints[0],
            "modified_time_ns": invalid_source_fingerprints[0]["modified_time_ns"] + 1,
        }
        return _service_smoke_tuple(
            executable=Path(executable),
            working_dir=working_dir,
    ...
    assert response.npm_install_smoke.serve_tag_files_replay is not None
    assert response.npm_install_smoke.serve_tag_files_replay.passed is True
    assert response.warnings == ["npm_tarball_serve_tag_files_replay_invalid_source_fingerprints"]

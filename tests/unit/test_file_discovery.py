...
def test_resolve_tag_file_sources_supports_directories_and_globs(tmp_path: Path) -> None:
    ...
    explicit_path.write_text("<p>Org Beta</p>", encoding="utf-8")
    nested_path.write_text("<p>EXCHX</p>", encoding="utf-8")
    globbed_path.write_text("<p>TICKA</p>", encoding="utf-8")
...
def test_discover_tag_file_sources_applies_include_exclude_filters_and_tracks_summary(
    ...
    keep_path.write_text("<p>Org Beta</p>", encoding="utf-8")
    drop_path.write_text("<p>EXCHX</p>", encoding="utf-8")
    generated_output.write_text("{}", encoding="utf-8")
    ...
    assert discovery.summary.discovered_input_bytes == (
        ...
        + len("<p>Org Beta</p>".encode("utf-8"))
        + len("<p>EXCHX</p>".encode("utf-8"))
    )
    assert discovery.summary.included_input_bytes == len("<p>Org Beta</p>".encode("utf-8"))
    assert discovery.summary.processed_input_bytes == len("<p>Org Beta</p>".encode("utf-8"))
...
def test_discover_tag_file_sources_applies_max_files_guardrail_in_discovery_order(
    ...
    first.write_text("<p>Org Beta</p>", encoding="utf-8")
    second.write_text("<p>EXCHX</p>", encoding="utf-8")
    third.write_text("<p>TICKA</p>", encoding="utf-8")

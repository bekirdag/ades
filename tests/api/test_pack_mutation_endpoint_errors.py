...
def test_activate_endpoint_returns_404_for_missing_explicit_config(
    ...
    response = client.post("/v0/packs/finance-en/activate")
...
def test_deactivate_endpoint_returns_400_for_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    response = client.post("/v0/packs/finance-en/deactivate")

    assert response.status_code == 400
    assert response.json()["detail"] == "Unsupported ades runtime target: 'broken-runtime'"


def test_remove_endpoint_returns_400_for_invalid_runtime_target(
    tmp_path: Path, monkeypatch
) -> None:
    config_path = tmp_path / "ades.toml"
    config_path.write_text("", encoding="utf-8")

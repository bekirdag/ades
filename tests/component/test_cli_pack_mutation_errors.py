...
def test_cli_activate_pack_reports_missing_explicit_config(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_cli_deactivate_pack_reports_invalid_runtime_target(tmp_path: Path) -> None:
    runner = CliRunner()
...
def test_cli_remove_pack_reports_invalid_runtime_target(tmp_path: Path) -> None:
    runner = CliRunner()
    config_path = tmp_path / "ades.toml"
    config_path.write_text("", encoding="utf-8")

...
Planned work:
- extend installed-artifact clean-environment validation from live service text tagging to live `/v0/tag/file` requests after finance-pack pull and metadata recovery
- keep validating repeated idempotent pack operations and lifecycle recovery behavior against published registry artifacts
- keep the new rollback-safe install semantics, lookup-driven metadata repair, SQLite recovery behavior, dependency-bearing release smoke, and live `/v0/tag` smoke stable while finishing the remaining pack-lifecycle recovery work
